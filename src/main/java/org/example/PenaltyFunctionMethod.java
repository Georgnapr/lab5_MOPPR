...
public class PenaltyFunctionMethod {
    ...
    private static final double OUTER_STAGNATION_FACTOR = 1e-3;
    private static final double INNER_STEP_FACTOR = 1e-3;
    private static final double INNER_PHI_FACTOR = 1e-6;
    ...
    public PenaltyMethodResult solve(PenaltyMethodRequest request) {
        ...
        double previousAlpha = Double.POSITIVE_INFINITY;
        int k = 0;
        ...
        while (true) {
            ...
            boolean constraintSatisfied = alpha <= epsilon;
            boolean stagnated = Math.abs(previousAlpha - alpha) <= epsilon * OUTER_STAGNATION_FACTOR;

            String comment = constraintSatisfied
                    ? "Штрафное условие выполнено"
                    : stagnated
                    ? "Остановка по стагнации"
                    : "Минимизация Phi(x, μk) методом сопряжённых градиентов";
            iterations.add(buildIteration(k, mu, x1, x2, gradientNorm, comment));

            if (constraintSatisfied || stagnated) {
                break;
            }
    ...
    private Point minimizePhiByConjugateGradients(double startX1, double startX2, double mu, double epsilon) {
        ...
        double d1 = -g1;
        double d2 = -g2;
        double previousPhi = Double.POSITIVE_INFINITY;
        int iteration = 0;
        ...
        while (true) {
            ...
            double newPhi = phi(newX1, newX2, mu);

            if (Math.hypot(newX1 - x1, newX2 - x2) <= epsilon * INNER_STEP_FACTOR
                    || Math.abs(previousPhi - newPhi) <= epsilon * INNER_PHI_FACTOR) {
                x1 = newX1;
                x2 = newX2;
                break;
            }
